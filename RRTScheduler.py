...
def collides_line(start_point, finish_point, circles):
    for circle in circles:
        k = (finish_point[1] - start_point[1]) / (finish_point[0] - start_point[0])
        b = start_point[1] - k * start_point[0]
        d = (pow((2 * k * b - 2 * circle['X'] - 2 * circle['Y'] * k), 2) - (4 + 4 * k ** 2) * (
                    b ** 2 - (circle['R']) ** 2 + (circle['X']) ** 2 + circle['Y'] ** 2 - 2 * circle['Y'] * b))
        if d >= 0:
            return True
...
trivial = Parser('Data/cheese.json')
